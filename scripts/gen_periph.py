...
def gen_i2c(i2c_periph: str, speed_hz: int, scl: str, sda: str,
    ...
    lines.append("")
    # F-194 (WB-20260927-02 T3, 销 WB-05 L-4): 帮手自足性走路线 (b) — 降级为
    # 不依赖 error_chain 的返回形态 (int: 0=OK, 负值=超时码, 诊断语义入注释)。
    # 旧输出引用 error_chain_t/ERR_PLAIN(×6)/ERR_OK 共 8 处且零定义零 include,
    # 全仓亦无该类型定义面 (幽灵契约), 照抄即编译失败; F-078 烟测 stub 恰好
    # 供定义掩盖此病。#include <stdint.h> 自发射 (仿 gen_usart unistd 自足
    # 先例 F-131): uint8_t/uint32_t 是帮手自身所需。
    lines.append("/* 4. Poll write helper — returns 0 = OK, negative = timeout code")
    lines.append(" *   (self-contained: no external error-chain dependency) */")
    lines.append("#include <stdint.h>")
    lines.append(f"static int i2c{i2c_n}_write(uint8_t dev_addr, uint8_t reg, uint8_t data) {{")
    lines.append("    uint32_t timeout = 100000;")
    lines.append(f"    while ({i2c_periph}->SR2 & (1<<1)) {{        // wait BUSY=0")
    lines.append("        if (--timeout == 0) return -1;   // I2C BUSY timeout")
    lines.append("    }")
    lines.append(f"    {i2c_periph}->CR1 |= (1<<8);                 // START")
    lines.append("    timeout = 100000;")
    lines.append(f"    while (!({i2c_periph}->SR1 & 1)) {{           // wait SB")
    lines.append("        if (--timeout == 0) return -2;   // I2C START timeout")
    lines.append("    }")
    lines.append(f"    {i2c_periph}->DR = (dev_addr << 1);          // ADDR + W")
    lines.append("    timeout = 100000;")
    lines.append(f"    while (!({i2c_periph}->SR1 & (1<<1))) {{       // wait ADDR")
    lines.append("        if (--timeout == 0) return -3;   // I2C ADDR timeout")
    lines.append("    }")
    ...
    lines.append("        if (--timeout == 0) return -4;   // I2C TXE timeout")
    lines.append("    }")
    ...
    lines.append("        if (--timeout == 0) return -5;   // I2C BTF timeout")
    lines.append("    }")
