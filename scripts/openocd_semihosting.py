...
def main() -> None:
    ...
    parser.add_argument("--reset", action="store_true", help="capture 前 reset halt 目标，保证 boot 从确定性起点开始 (默认仅 halt 续跑)")
    ...
    try:
        ...
        try:
            # --reset: 确定性起点 — 上一会话可能在 printf 的 BKPT 冻结或卡在 boot 中段,
            # 仅 halt 续跑会得到不完整的 boot 输出。reset halt 后从 boot 重新开始。
            if args.reset:
                telnet_send_multi("localhost", int(telnet_port or 4444), ["reset halt", "arm semihosting enable", "resume"])
            else:
