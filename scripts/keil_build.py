...
def _echo_log_problems(log_file: str) -> None:
    """非 JSON 模式: 把构建日志中的 ARMCC 错误/警告行回显到 stderr。

    供 VS Code tasks.json 的 armcc-v5 problemMatcher 解析 (`file(line): error|warning: ...`)。
    仅 human 模式调用；--json 模式 stdout 必须保持纯 JSON（verify.py 依赖）。
    """
    ...
    try:
        with open(log_file, "r", encoding="utf-8", errors="replace") as file_obj:
            for line in file_obj:
                if re.search(r"\(\d+\)\s*:\s*(error|warning)\s*:", line, re.IGNORECASE):
...
def main() -> None:
    ...
    try:
        ...
        # log_dir: CLI > 工程级配置 > 环境级配置 > 默认值(.workbench/build)
        log_dir_raw = args.log_dir or project_config.get("log_dir") or local_config.get("log_dir")
        log_dir = _resolve_workspace_path(workspace, log_dir_raw, ".workbench/build")
        if args.log_dir:
    ...
    if result["status"] == "ok":
        ...
        if result.get("details", {}).get("log_file"):
            ...
            _echo_log_problems(result["details"]["log_file"])
        sys.exit(1)
