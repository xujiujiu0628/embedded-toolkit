...
- 提示词模板必须包含规格 §4 四条共同铁律的语义等价表述（允许为可执行性扩写，
  审计 L2：不要求逐字引用）
...
（深度审核/审计/fresh checker）；四条共同铁律语义等价出现；两套模板各自含五个维度；
...
- [ ] **调度验收**：skill 写入后当会话即时注册属实，但注册 ≠ 流程可用——
  原口径"用户触发并严格走完四步"于 **2026-08-26 晚由首次真实运行达成**
  （用户说"审核一下"→ impl 模式完整四步 → 报告交付+落账 ev_203604）。
  该次运行同时暴露并修复了 Step 4 的 cwd 陷阱等问题，见审计记录。
...
## 附录：实战派发提示词存档（审计 M-4 回填——"同源可比对"的比对基准）

### 样本一：design 模式（2026-08-26 午，harness-borrow 设计审，11 发现）

```text
你是一名资深的嵌入式工具链架构评审员。你此前从未接触过这个项目，请以全新、
怀疑的视角对下面这份设计提案做对抗性评审。你的职责是找问题，不是背书。
【铁律：本次评审为只读任务。禁止修改、创建、删除任何文件。】

# 待评审设计（全文照录）
<设计方在讨论中已与用户达成以下方案，现在请你冷审：>
## 目标 / ## 设计第 1~4 节 / ## 明确不做
<设计全文照录于提示词内>

背景事实（可自行到磁盘核实）：
- 工具库在 <工作区根>\embedded-toolkit（git 仓），核心脚本 scripts/verify.py …
- verify.py 目前从工程 .workbench/config.json 读 verify.expect … --json 可输出结果
- 活跃工程：adc-oled 和 button-toggle（GCC+HAL 工作流，均无 git、无可执行测试）…
- RTT 是默认采集后端；用户是单人工作流，AI（Claude Code）写码为主

# 你的评审任务
1. 先读真实代码核对设计假设（这是重点，不要凭空评）：
   - <工作区根>\embedded-toolkit\scripts\verify.py —— 重点看：是否脚本风格直通、
     期望匹配逻辑现状、--json 输出现状；
   - <工作区根>\stm32f103-adc-oled\.workbench\config.json —— legacy 格式真貌；
   - <工作区根>\Embedded-AI-Harness\.claude\skills\build\SKILL.md —— 被借鉴机制的原文语境；
2. 对抗性找茬，至少覆盖：schema 缺口 / XPASS 取舍 / G1 重烧合理性 /
   legacy 双格式成本 / 纯函数拆分可行性 / YAGNI 反向审查 / 安全风险（自动打 tag）；
3. 输出格式（中文）：总体结论一句话 → 发现列表按严重度【Critical/High/Medium/Low】
   （问题+证据 文件:行号+建议）→ "设计中被确认可行的部分"（简短）。
你的最终回复就是评审报告本身，直接给结论和证据，不要寒暄。
```

### 样本二：impl 模式（2026-08-26 傍晚，harness-borrow 实现审，3M+6L）

```text
你是一名资深嵌入式工具链审计员，此前从未接触本项目。请以全新、怀疑的视角
对一份刚完成的实现做深度审核。你的职责是找问题、核实声称，不是背书。
【规则】只读审计——禁止修改/创建/删除任何文件；但允许运行测试和只读命令取证。

# 审计对象
<交付声称全文：两个 git 仓、各 commit 范围、声称的交付清单、已知遗留>

# 材料指针
- 规格文件路径 / 计划文件路径 / canonical 与镜像文件路径 / 相关 commit 范围 /
  反馈数据产物路径 / 可运行验证示例

# 你的审计维度（逐项给证据）
1. 规格符合度：规格逐条对照实现，指出无对应实现的条款；
2. 代码正确性：写边界探针实测（非法输入/异常路径/组合语义），引用运行输出；
3. 测试充分性：高风险路径与已测集合的差集，漏测即发现；
4. 产物可信度：哈希重算、tag 指向核对、commit 叙事一致性；
5. 文档债：checkbox/README/spec 与实现有无漂移。

# 输出格式（中文）
总体结论一句话 → 分级发现【Critical/High/Medium/Low】×N（问题+证据+建议）→
确认合格清单 → 关键验证命令及结果摘要表。直接给报告，不要寒暄。
```

注：样本二的骨架当晚已被吸收为 SKILL.md 的 impl 模板正文；fresh-checker 首轮
自审计（第三轮）的派发提示词即该模板 + 材料区的直接实例化，不再重复存档。
