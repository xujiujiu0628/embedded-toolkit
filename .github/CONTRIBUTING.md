...
- **Agent（含代管智能体）不得直接 push 远端**：本地 commit → 维护者审核 →
  由维护者推送，或经维护者明确授权后方可执行
- 推送前检查：全量测试绿、工作区干净、CHANGELOG 引用 hash 可解析
  （历史重写后尤甚，见 F-051）
- force-push 仅限维护者执行；Agent 有需求时说明理由待批
- **本节性质（F-065 标注）**：本节为**道德约束**——本仓无任何 git hook /
  pre-push / CI 阻断逻辑会拦截 Agent 推送，技术兜底依赖：
  1. **GitHub 远端分支保护**（维护者 web 端配置：master 需 PR + ≥1 审），
     维护者必须确认已配置；如未配置，Agent 一旦 push 仍可直达 master
  2. **维护者人工审核**（Agent 必须先汇总变更给维护者，等待"通过"回复后方可授权推送）
  若你读到本节以为是"已卡死"，请回到本段——它是约定，不是卡点
